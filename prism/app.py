...
async def api_geo_optimize(request: Request):
    ...
    if lang == "zh-TW":
        geo_prompt = (
            "你係生成式引擎優化（GEO）專家，專精 B2B 專業內容，尤其係合規同網絡安全議題。"
            "請根據以下原則改寫內容：\n\n"
            "核心原則：\n\n"
            "時效性與準確性\n"
            "- 必須反映現行法律或監管狀態\n"
            "- 唔好將已發生嘅事寫成未來，亦唔好用過時術語（例如將已通過嘅法案叫做法案草案）\n\n"
            "適用範圍精準度\n"
            "- 清楚區分邊啲對象直接受監管、邊啲只係間接受影響\n"
            "- 唔好誇大法規嘅適用範圍\n\n"
            "結構便於提取\n"
            "- 用清晰、匹配搜尋查詢嘅標題\n"
            "- 短段落同編號／項目符號列表，方便 AI 模型提取重點\n"
            "- 建議要具體、可行動\n\n"
            "權威但唔硬銷\n"
            "- 用可信來源支持論述（如有）\n"
            "- 將公司定位為專業顧問，唔係推銷員\n"
            ...
            "- 用原文語言輸出\n"
            "- 保持相似長度同整體結構\n"
            "- 結尾用輕柔、專業嘅方式提及相關服務，而非強硬推銷\n"
            "- 純文字，唔好用 markdown 格式\n\n"
            f"原始內容：\n\n{content}"
        )
    else:
        geo_prompt = (
            "You are an expert in Generative Engine Optimization (GEO) for B2B "
            "professional content, especially regulatory and cybersecurity topics. "
            "Rewrite the following content according to these principles:\n\n"
            "Core Principles:\n\n"
            "Timeliness & Accuracy\n"
            "- Always reflect the current legal or regulatory status\n"
            "- Never present past events as future, or use outdated terminology "
            "(e.g. calling an enacted law a 'bill')\n\n"
            "Scope Precision\n"
            "- Clearly distinguish who is directly regulated vs indirectly affected\n"
            "- Avoid overstating the applicability of the law or regulation\n\n"
            "Structure for Extractability\n"
            "- Use a clear, query-matching title\n"
            "- Prefer short paragraphs and numbered/bulleted lists so models can "
            "easily extract key points\n"
            "- Keep recommendations concrete and actionable\n\n"
            "Authority without Hard Selling\n"
            "- Support claims with credible references when available\n"
            "- Position the company as a helpful expert resource, not a sales pitch\n"
            "- Soften any call-to-action so it feels natural and secondary\n\n"
            "Tone\n"
            "- Prioritise being helpful and professional\n"
            "- Commercial intent should remain light and integrated, never dominant\n\n"
            "Output Style:\n"
            "- Rewrite in the same language as the original\n"
            "- Maintain similar length and overall structure\n"
            "- End with a soft, professional mention of relevant services rather "
            "than an aggressive closing line\n"
...
@app.post("/api/trending-hk")
async def api_trending_hk(request: Request):
    """Fetch the top 5 currently trending searches in Hong Kong via web search."""
    import asyncio
    lang = _resolve_lang(request)
    ...
    match = re.search(r"\[.*\]", raw, re.S)
    if not match:
        return JSONResponse({"error": "LLM did not return a valid list"}, 500)
    try:
        keywords = json.loads(match.group(0))
        if isinstance(keywords, list):
