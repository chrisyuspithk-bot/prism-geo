...
PERIODS = {"weekly": (7, "Weekly"), "monthly": (30, "Monthly")}
...
def generate(tenant_id: int, tenant_name: str, brand_name: str,
             period: str, model_id: int | None = None) -> bytes:
    days, label = PERIODS.get(period, PERIODS["monthly"])
    with connect() as conn:
        data = _build_data(conn, tenant_id, days, model_id)

    now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    title = f"GEO Visibility Report — {brand_name}"
